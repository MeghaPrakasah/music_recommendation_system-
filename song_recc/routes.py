...
from song_recc import app,db
from song_recc import app
from song_recc.models import *
...
import numpy as np
import pickle
from song_recc import Song_Recommendation
# from yoga import chatbot
from tkinter import *
...
def add_teachers():
    if request.method == 'POST':
        ...
        my_data = Teachers(name=name,username=username,contact=contact,password=password,usertype="teacher")
...
def add_students():
    if request.method == 'POST':
        ...
        my_data = Students(name=name,username=username,contact=contact,sid=sid)
        db.session.add(my_data)
        db.session.commit()
...
def view_report():
    ...
    if request.method == 'POST':
        ...
        print("---------------")
        print(str(sdate))
        obj = Detection.query.filter(Detection.det_date > sdate, Detection.det_date < edate).all()
        # obj=Detection.query.filter_by(det_date>sdate,det_date>edate).all()
        return render_template("view_report.html",obj=obj)
...
def view_reports():
    ...
    if request.method == 'POST':
        ...
        obj = Detection.query.filter(Detection.det_date > sdate, Detection.det_date < edate).all()
        # obj=Detection.query.filter_by(det_date>sdate,det_date>edate).all()
        return render_template("view_reports.html",obj=obj)
...
# @app.route('/view_reports',methods=["GET","POST"])
# def view_reports():
#     obj=Detection.query.all()
#     objj=Students.query.filter_by(sid=obj.sid).first()
#     return render_template("view_reports.html",obj=obj,objj=objj)
...
def user_register():
    if request.method == 'POST':
        ...
        contact = request.form['contact']
        password = request.form['password']
        my_data = Login(name=name,username=email,contact=contact,password=password,usertype="user")
        db.session.add(my_data) 
        db.session.commit()
...
def detection():
        ...
        if request.method == 'POST':
            d = Song_Recommendation.song_recommendations()
            sid = request.form['sid']
            subject = request.form['subject']
            today = date.today()
            print(today)
            ...
            my_data = Detection(sid=sid,subject=subject,emotion=d,det_date=today,time=current_time)
            db.session.add(my_data) 
            db.session.commit()
...
@app.route('/happy_emoji',methods=['GET', 'POST'])
def happy_emoji():
    return render_template("happy_emoji.html")



@app.route('/sad_emoji',methods=['GET', 'POST'])
def sad_emoji():
    return render_template("sad_emoji.html")


@app.route('/disgust_emoji',methods=['GET', 'POST'])
def disgust_emoji():
    return render_template("disgust_emoji.html")


@app.route('/fear_emoji',methods=['GET', 'POST'])
def fear_emoji():
    return render_template("fear_emoji.html")


@app.route('/angry_emoji',methods=['GET', 'POST'])
def angry_emoji():
    return render_template("angry_emoji.html")

@app.route('/surprised_emoji',methods=['GET', 'POST'])
def surprised_emoji():
    return render_template("surprised_emoji.html")


@app.route('/neutral_emoji',methods=['GET', 'POST'])
def neutral_emoji():
    return render_template("neutral_emoji.html")


@app.route('/manage_doctor')
def manage_doctor():
    d=Doctor.query.all()
    return render_template("manage_doctor.html",d=d)
...
def chat_bot():
    ...
    from song_recc.Classes import Preprocessing, FeatureExtraction, Models
    ...
    desktop_path = 'C:/Users/Rajasree/Desktop/song_recc/yoga/SavedModels'
...
def login():
    ...
    if request.method=="POST":
        ...
        teacher=Teachers.query.filter_by(username=username,password=password, usertype= 'teacher').first()
        if teacher:
            login_user(teacher)
            next_page = request.args.get('next')
            return redirect(next_page) if next_page else redirect('/user_index/'+str(teacher.id)) 
        elif admin:
            login_user(admin)
            next_page = request.args.get('next')
            return redirect(next_page) if next_page else redirect('/admin_index/'+str(admin.id)) 
